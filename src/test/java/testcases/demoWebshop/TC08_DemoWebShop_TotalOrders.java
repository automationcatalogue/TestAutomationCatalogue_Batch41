...
public class TC08_DemoWebShop_TotalOrders extends TestRunner {
    ...
    public void tearDown(ITestResult result) throws Exception {
        if (result.getStatus() == ITestResult.SUCCESS) {
            ...
            logger.log(Status.PASS, "DemoWebShop Total Orders TestCase is Passed and status is updated in Excel sheet");
            ...
            if (!BaseClass.failureReason.equalsIgnoreCase("TestId is not found")) {
                ...
                logger.log(Status.FAIL, "DemoWebShop Total Orders TestCase is Failed and status is updated in Excel sheet");
            }
        } else if (result.getStatus() == ITestResult.SKIP) {
            ExcelUtils.setCellData("SKIPPED", "Index", rowNum_Index, Config.col_Status);
            log.info("TestCase is SKIPPED and status is updated in Excel sheet");
            logger.log(Status.SKIP, "DemoWebShop Total Orders TestCase is Skipped and status is updated in Excel sheet");
        }
